...
public:
    ...
    bool Connect(std::string ipaddr, int port){
        ...
        while (true) {
            ...
            std::cout << "Enter b: ";
            std::cin >> req.b;
            sent_bytes_ = sendto(sock_fd_, &req, sizeof(req), 0, (sockaddr *) &server_addr_, sizeof(sockaddr));
            std::cout << "No. of bytes sent: " << sent_bytes_ << std::endl;
            Response res;
            recv_bytes_ = recvfrom(sock_fd_, &res, sizeof(res), 0, (sockaddr *) &server_addr_, (socklen_t *) &server_addr_len_);
